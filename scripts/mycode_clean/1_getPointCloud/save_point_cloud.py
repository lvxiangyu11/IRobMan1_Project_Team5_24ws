...
class PointCloudSaver:
    def __init__(self):
        ...
        rospy.Subscriber(
            "/zed2/zed_node/left/image_rect_color", Image, self.image_callback
        )  # Image callback
        rospy.Subscriber(
    ...
    def save_point_clouds(self, world_path):
        ...
        point_list = pc2.read_points(
            self.point_cloud_data, field_names=("x", "y", "z", "rgb"), skip_nans=True
        )
        points = []
        colors = []
        ...
        rospy.loginfo(
            f"Color range - Min: {np.min(colors_array, axis=0)}, Max: {np.max(colors_array, axis=0)}"
        )
        ...
        # transformed_point_cloud = self.transform_point_cloud(
        #     transformed_point_cloud, colors, transform2
        # )
    ...
    def transform_point_cloud(self, points, colors, transform):
        ...
        transformation_matrix[:3, :3] = (
            o3d.geometry.get_rotation_matrix_from_quaternion(rotation)
        )
