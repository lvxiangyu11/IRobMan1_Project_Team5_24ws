...
class PointCloudSaver:
    ...
    def save_point_clouds(self, original_path, world_path):
        ...
        if self.color_image is not None:
            color_image_path = os.path.splitext(original_path)[0] + ".png"  # 使用相同的文件名但扩展名为.png
            cv2.imwrite(color_image_path, self.color_image)  # 使用OpenCV保存彩色图像
            rospy.loginfo(f"Color image saved to {color_image_path}")
        else:
            rospy.logwarn("No color image available to save.")
